...
import com.Biblioteca.Biblioteca.modelo.Libros;
import com.Biblioteca.Biblioteca.modelo.Usuario;

@Repository
public interface LibrosRepositorios extends JpaRepository<Usuario, Integer> {

    @Query("SELECT u FROM Usuario u WHERE u.nombre LIKE %?1%"
    + " OR u.apellido LIKE %?1%"
    + " OR u.fechaNacimiento LIKE %?1%")
    public List<Libros> findAll(String palabraClave);
